...
See [Lifecycle of a Chrome Web Store item](https://developer.chrome.com/docs/webstore/check-review#the_lifecycle_of_a_chrome_web_store_item) for an overview of how reviews fit in the
lifecycle.
...
with the [developer program policies](https://developer.chrome.com/docs/webstore/program-policies) and, if any violations are found, take appropriate
[enforcement actions](https://developer.chrome.com/docs/webstore/review-process#enforcement).
...
| **Note:** If your extension is pending review for more than three weeks, please [contact developer support](https://developer.chrome.com/docs/webstore/review-process#support) to request assistance.
...
| **Note:** Note that all item submissions---whether for a new item or an update to an existing one---are subject to the same review process.
...
:   [Host permissions](https://developer.chrome.com/docs/extensions/mv3/declare_permissions) patterns like `*://*/*`, `https://*/*`, and `<all_urls>` give
...
| **Note:** Existing published items may also be checked for these same violations; that process is described in the [Periodic review outcomes](https://developer.chrome.com/docs/webstore/review-process#periodic-outcomes) section.
...
- **A violation is found:** The submission is [rejected](https://developer.chrome.com/docs/webstore/review-process#rejection) and the developer is informed why.

See [Developer communication](https://developer.chrome.com/docs/webstore/review-process#comms) for details on how these outcomes are communicated back
...
extreme policy violation. See the [malware](https://developer.chrome.com/docs/webstore/review-process#malware) section for details on how these
...
that a violation identified during the [publishing review](https://developer.chrome.com/docs/webstore/review-process#publish-outcomes) process may
...
review outcomes' and 'Violation enforcement' sections.](https://developer.chrome.com/static/docs/webstore/review-process/image/illustration-potentia-9c316ca43b57a.png)
...
- **A minor violation is found:** A warning is sent to the developer about the violation. The developer has a set amount of time to address the violation before the item will be taken down. See [warning](https://developer.chrome.com/docs/webstore/review-process#warning) for more information.
- **A more serious violation is found:** The extension is immediately taken down and the developer is notified of the violation. See [takedown](https://developer.chrome.com/docs/webstore/review-process#takedown) for more information.
- **An extreme issue is found:** The extension is immediately taken down and the developer **is
  not** notified. See [malware](https://developer.chrome.com/docs/webstore/review-process#malware) for more information.

See [Developer communication](https://developer.chrome.com/docs/webstore/review-process#comms) for details on how this is communicated and how to
appeal the verdict.
...
| **Note:** Rejection applies only when the submission is not found to contain malware or other extreme policy violation. If an extension is found to contain malware during the submission review process, proceed directly to [malware enforcement](https://developer.chrome.com/docs/webstore/review-process#malware).
...
The following information only covers the warning period. See [Takedown](https://developer.chrome.com/docs/webstore/review-process#takedown) for
...
:   The publisher email address associated with the extension will be sent a warning email stating
    ...
    taken down. See [Developer communication](https://developer.chrome.com/docs/webstore/review-process#comms) for additional information about email
    communication.
...
Second, delayed takedowns occur after the [warning period](https://developer.chrome.com/docs/webstore/review-process#warning) for a minor policy
...
Chrome Web Store listing
:   Just as with a [takedown](https://developer.chrome.com/docs/webstore/review-process#takedown), the offending item is removed from the Chrome Web
    Store.
...
Support tickets must be opened using the [One Stop Support](https://developer.chrome.com/docs/webstore/review-process#support) form, but once a
...
The [One Stop Support contact form](https://support.google.com/chrome_webstore/contact/one_stop_support) provides Chrome Web Store publishers with a
...
Use the following steps to appeal a [takedown](https://developer.chrome.com/docs/webstore/review-process#takedown) or [warning](https://developer.chrome.com/docs/webstore/review-process#warning).

1. Open the [One Stop Support contact form](https://support.google.com/chrome_webstore/contact/one_stop_support).
...
about your issue.
| **Note:** As a developer in the European Union, if you have concerns related to the Platform To Business (P2B) regulations in Europe, you can raise them by filing through the [One Stop Support contact form](https://support.google.com/chrome_webstore/contact/one_stop_support).
